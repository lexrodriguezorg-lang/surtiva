...
export function animateBrand({force=false}={}){
 ...
 overlay.innerHTML=brand+'<video muted playsinline preload="auto" aria-label="Animación original de la marca SURTIVA"><source src="/brand/intro-original.mp4" type="video/mp4"></video><button type="button" class="brand-film-skip">Entrar <span aria-hidden="true">↗</span></button>';
 ...
 const loadTimeout=setTimeout(finish,8000),hardTimeout=setTimeout(finish,16000);
 video.muted=true;video.addEventListener('playing',()=>{clearTimeout(loadTimeout);overlay.classList.add('playing');try{sessionStorage.setItem('surtiva.film.seen.v1','1');}catch{}},{once:true});video.addEventListener('ended',finish,{once:true});video.addEventListener('error',finish,{once:true});
