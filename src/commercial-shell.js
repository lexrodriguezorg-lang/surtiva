...
// This is today's commercial scope. ERP tools remain in Surtiva administration.
export function commercialNav(role){
 if(role==='distributor_admin')return [['catalogo','Mi catálogo','grid'],['pedidos','Pedidos de Surtiva','bag']];
...
export function commercialRoute(role,page){
 const nav=commercialNav(role);
 if(page==='visitas'&&role==='seller')return 'clientes';
 return nav.some(([id])=>id===page)?page:nav[0][0];
}
export function commercialShell(content,{role,page,avatar,name='',canRead=()=>true}){
