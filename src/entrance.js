...
export function entranceMarkup(){return `<div class="surtiva-entrance">
 ...
 <main id="contenido" class="entrance-main">
  ...
  <section class="entrance-showcase toys" aria-label="Una mirada al surtido de SURTIVA">
   ...
   <figure class="showcase-main"><img src="/assets/productos/${collections[0].image}" alt="${collections[0].alt}" fetchpriority="high" width="384" height="384"><figcaption><span>Juguetería</span><b>01 / 03</b></figcaption></figure>
...
document.addEventListener('click',event=>{
 ...
 if(collection){const i=Number(collection.dataset.collection),c=collections[i],stage=document.querySelector('.entrance-showcase');stage.className='entrance-showcase '+c.color;const picture=stage.querySelector('.showcase-main img');picture.src='/assets/productos/'+c.image;picture.alt=c.alt;stage.querySelector('figcaption span').textContent=c.name;stage.querySelector('figcaption b').textContent=`0${i+1} / 03`;stage.querySelector('.showcase-detail img').src='/assets/productos/'+c.next;document.querySelectorAll('[data-collection]').forEach(b=>b.setAttribute('aria-pressed',String(b===collection)));}
