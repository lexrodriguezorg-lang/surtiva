begin;
-- A bounded catalogue response. All visibility rules remain in PostgreSQL.
create function public.browse_catalog(options jsonb default '{}') returns jsonb
language plpgsql stable security definer set search_path='' as $$
declare result jsonb; skip integer:=least(greatest(coalesce((options->>'offset')::int,0),0),100000);
...
begin
 if not exists(select 1 from public.memberships m join public.organizations o on o.id=m.organization_id where m.user_id=auth.uid() and m.status='active' and o.status='active') then
...
revoke all on function public.browse_catalog(jsonb) from public,anon;
grant execute on function public.browse_catalog(jsonb) to authenticated;
...
begin
 ...
 if operation='catalog' then
  ...
  select coalesce(jsonb_agg(to_jsonb(t)),'[]') into products from (
   ...
   from (select source.*,row_number() over(partition by category order by md5(sku),id) discovery_rank from private.portal_products(a) source) p left join public.inventory i on i.organization_id=p.organization_id and i.product_id=p.id
  ...
  select coalesce(jsonb_agg(to_jsonb(t)),'[]') into cats from (select p.category as name,count(*) as count,(array_agg(p.image order by md5(p.sku),p.id) filter(where p.image is not null and p.image<>''))[1] as image from private.portal_products(a) p group by p.category order by p.category) t;
