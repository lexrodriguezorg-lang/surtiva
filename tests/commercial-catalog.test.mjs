...
test('all approved profiles see published products, while private business data remains isolated',async()=>{
 const db=await database();try{
  ...
  await asUser(db,'seller',async()=>{
   assert.equal((await db.query('select private.can_product($1,$2) ok',[ids.org,ids.product3])).rows[0].ok,true);assert.equal(await count(db,'clients'),1);
   const order=(await db.query('select public.create_order($1,$2,$3::jsonb) id',[ids.org,ids.customer,JSON.stringify([{productId:ids.product3,quantity:3}])])).rows[0].id;
   assert.equal((await db.query('select status from public.orders where id=$1',[order])).rows[0].status,'aprobacion');
   await assert.rejects(db.query('select public.create_order($1,$2,$3::jsonb)',[ids.org,ids.customer2,JSON.stringify([{productId:ids.product3,quantity:3}])]));
  });
